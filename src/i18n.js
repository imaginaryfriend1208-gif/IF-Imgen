...
const en = {
    ...
    lbl_workflow: 'Workflow (API format)', btn_wf_load: 'Load workflow .json', btn_wf_automap: 'Auto-map',
    ...
    help_steps: [
        ['Connect the image API', 'Settings → <b>1 · Image API</b>. Pick <i>A1111 / proxy</i> (A1111-compatible URL, optional user:pass), <i>ComfyUI</i> (ComfyUI URL + your workflow exported with <i>Save (API format)</i> → <b>Auto-map</b> inserts the %placeholders%; <code>&lt;lora:…&gt;</code> tags become LoRA nodes) or <i>NovelAI</i> (API key). Press <b>Test</b>, then <b>Set active</b>.'],
...
const vi = {
    ...
    lbl_workflow: 'Workflow (API format)', btn_wf_load: 'Nạp workflow .json', btn_wf_automap: 'Tự gắn placeholder',
    ...
    help_steps: [
        ['Kết nối API ảnh', 'Cài đặt → <b>1 · API ảnh</b>. Chọn <i>A1111 / proxy</i> (URL tương thích A1111, user:pass nếu có), <i>ComfyUI</i> (URL ComfyUI + workflow xuất bằng <i>Save (API format)</i> → bấm <b>Tự gắn placeholder</b>; tag <code>&lt;lora:…&gt;</code> tự thành node LoRA) hoặc <i>NovelAI</i> (API key). Bấm <b>Kiểm tra</b>, rồi <b>Kích hoạt</b>.'],
