...
function mountOnce({ root, settings, save, backends, llm, pipeline, getContext, viewer, discordUrl, kofiUrl, onLanguageChange, onCollapseChange, onAlignChange, onFloaterChange, onShowButtonChange }, openTab) {
    ...
    function mountEntityTab({ kind, tab }) {
        ...
        function load(e) {
            ...
            q('.ent-tags').value = e.tags; q('.ent-natural').value = e.natural; q('.ent-negative').value = e.negative; if (q('.ent-lead')) q('.ent-lead').value = e.lead ?? '';
        ...
        function renderProfile(e) {
            ...
            if (renderProfile.lastId !== (saved?.id ?? null)) { // new entry -> its stored choices; re-renders keep what the user picked
                q('.ent-profile-shot').value = prof.shot ?? 'portrait';
                q('.ent-profile-sfw').checked = prof.sfw !== false;
                renderProfile.lastId = saved?.id ?? null;
            }
...
function stylePanel({ tab, label, icon, hint }) {
    ...
    <div class="ifimgen-panel" data-panel="${tab}">
        ...
        <div class="ifimgen-box">
            ...
            <div class="ifimgen-row"><label>${t('lbl_lead')}</label><textarea class="text_pole ent-lead" rows="2" placeholder="${t('ph_lead')}"></textarea></div>
            <div class="ifimgen-note">${t('note_lead')}</div>
