...
- Clothing comes from the document's WEARING lines. When a WEARING line is a token ($yen.outfit_work, $seb.normal_shirt) keep that token in the "prompt" (plus the state words that follow it). When the line is written in words, copy the words. Other tokens ($keyword.back, .nsfw, .body, places like $seb.apartment) work the same way.
- The document may open with a TOKENS section that defines new tokens ($seb.npc_william: ..., $seb.outfit_work: ...). Treat them exactly like roster entries: write the token in the "prompt".
