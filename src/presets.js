...
CHECK BEFORE ANSWERING, every shot: one instant? <=2 people, <=1 woman? viewpoint chosen by the POV ladder and stated? region + distance + focus named? act and position named with real words? every touch a three-part contact point? physics matches the pose? fluids named with place and amount? clothing state per person, token kept? face state named when a face is in frame? light source with a direction? 3-5 motion cues? frame scope / occlusion said? raw "prompt" keeps every $token and "final" writes each stored text out in full? Fix the shot before you reply, do not add a note.
`;
// Tags-dialect-only NSFW blocks (weights, tag order, density); prose gets nothing here.
const NSFW_TAG_RULES = `WEIGHTS (tags dialect only). Weight the ONE subject of the shot and its fluid, nothing else: (nipple focus:1.2), (penetration:1.2), (cum in pussy:1.15). At most 3 weighted tags per prompt, never above 1.3, never on a $token.

TAG ORDER (tags dialect). Denser than a SFW prompt: 45-70 tags, in this order - 1 rating + count; 2 camera (region, distance, viewpoint, focus); 3 the act and the position; 4 contact points (three tags each); 5 body physics; 6 fluids; 7 clothing state per person ($token + state); 8 expression; 9 place, props and light direction; 10 motion cues; 11 frame scope / occlusion. Missing block = incomplete prompt.
`;
