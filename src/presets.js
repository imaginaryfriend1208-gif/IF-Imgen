...
- "final" (smooth): the same shot written for the image model, with NO tokens at all. Replace each $keyword by the person's name or role plus ONLY the traits of their base look that this shot actually shows (4-6 traits: a close-up of hips and hands shows no face, so no eyes, no lashes, no mole; from behind shows no expression; a covered head shows no hair), and replace each $keyword.key by the visible part of its text, trimmed and reworded so the sentence reads naturally. Skip traits and entry text that are out of frame, hidden or irrelevant to the shot. The "final" is what gets drawn - it must stand alone and stay inside the length rule below.
...
BODY PHYSICS. Flesh reacts: breasts pressed flat / squeezed / bouncing, skin indentation under a grip, ass cheeks spread or squeezed, thighs quivering, belly tensed, back arched, toes curled, blush spreading over chest, veins standing out on a forearm.
...
EXPRESSION UNDER SEX. Every shot that shows a face names its state: ahegao, torogao, fucked silly, eyes rolling back, half-closed eyes, cross-eyed, biting lip, tongue out, open mouth, moaning, screaming, tears, drool, blush, smug, looking at viewer, looking at partner, eye contact, embarrassed, nervous, tense.
...
export function renderPlannerPrompt(preset, a) {
    ...
    const user = [
        a.roster ? `ROSTER (keyword -> who they are, base look, stored entries as $keyword.key: text. In the raw "prompt" use the tokens; in the "final" write the visible traits and entry texts out in words):\n${a.roster}` : 'ROSTER: (none)',
