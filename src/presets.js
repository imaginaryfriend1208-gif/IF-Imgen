...
ONE VERSION of every prompt: "prompt" keeps every $keyword and $keyword.key token exactly as listed. Do NOT write a person's base look or a stored entry's text yourself - the token stands for it and is replaced by the stored text, word for word, after you answer. A token you spell out or paraphrase is a duplicate; a stored fact you leave out of the token is lost.
...
[{"p": <paragraph number>, "prompt": "<prompt with $tokens>"}]
- "p" must be one of the paragraph numbers listed, each used at most once. Pick the most visual moments.
- "prompt" is required for every object; no other text fields.
 ...
 */
const DOC_LENGTH = {
    tags: 'With a document the prompt is longer than usual: 40-65 tags, because place, props, clothing state, expression and pose of every person are all covered (a $token counts as one tag).',
    natural: 'With a document the paragraph is longer than usual: 110-170 words, because place, props, clothing state, expression and pose of every person are all covered (a $token counts as one word).',
};
const DOC_EXAMPLES = {
...
CHECK BEFORE ANSWERING, every shot: one instant? <=2 people, <=1 woman? viewpoint chosen by the POV ladder and stated? region + distance + focus named? act and position named with real words? every touch a three-part contact point? physics matches the pose? fluids named with place and amount? clothing state per person, token kept? 3-5 expression cues for EVERY face in frame, his included, matching what that person feels? light source with a direction? 3-5 motion cues? frame scope / occlusion said? "prompt" keeps every $token as listed, nothing spelled out? Fix the shot before you reply, do not add a note.
`;
...
const NSFW_EXAMPLES = {
    tags: `EXAMPLES (tags dialect - three kinds of shot: contact close-up / wide position / face; "prompt" keeps the $tokens; they are replaced by the stored texts afterwards):
[
 ...
 {"p": 5, "ar": "portrait", "prompt": "close-up, face focus, from above, pov, $mara looking up at viewer, ahegao, eyes rolling back, tears in eyes, drool string from open mouth, flushed cheeks, cum on face, cum on tongue, cum string, messy hair stuck to sweaty forehead, trembling, harsh overhead light, shallow depth of field"}
]
=====`,
    natural: `EXAMPLES (prose dialect - three kinds of shot: contact close-up / wide position / face; "prompt" keeps the $tokens; they are replaced by the stored texts afterwards):
[
...
- EVERY prompt must contain, explicitly:
  ...
  (b) for EACH person present in that shot: their $keyword, then their clothing COPIED from the document's "wearing" line for that person - the clothing TOKEN exactly as the document writes it ($kenjou wearing $kenjou.outfit_casual, collar open) plus the state words that follow it; when the document writes the clothing in words, copy those words with their colours. Never paraphrase, shorten or re-describe a token's stored text and never invent a garment the document does not have. Then their EXPRESSION and gaze, the action of that paragraph with its points of contact, and their POSE / POSITION relative to the furniture and to the other people.
- CLOTHING IS NEVER OPTIONAL: every person whose body below the neck is in the frame carries their clothing token (or the document's explicit undressed state: shirtless, naked, bra pushed up) in EVERY shot - close-ups, hand shots and shots from behind included. A person with no clothing statement is drawn at random by the image model; that is the one thing that must not happen.
- Other tokens ($keyword.back, .nsfw, .body, hairstyles, places like $seb.apartment, $world.xxx) work the same way: write the token, add only what differs right now.
- The document may open with a TOKENS section and the prompt may list CHAT TOKENS: both define tokens exactly like roster entries - write the token in the "prompt", never its text.
 ...
 *   tokens   - "$key.facet: text" lines of chat tokens the document refers to but does not define itself (ledger)
 */
export function renderPlannerPrompt(preset, a) {
    ...
    const what = doc
        ? 'translate the SCENE DOCUMENT into the prompt of each (place + props from LAYOUT, lighting, and for every person present: their clothing token from the document plus its state - never spelled out, never left out - expression, the action of that paragraph, pose / position)'
    ...
    const user = [
        a.roster ? `ROSTER (keyword -> who they are, base look, stored entries as $keyword.key: text. Write the tokens in the "prompt"; they are replaced by these texts afterwards):\n${a.roster}` : 'ROSTER: (none)',
        doc ? `SCENE DOCUMENT (authoritative for this reply - translate it into the prompts):\n${doc}` : '',
        doc && a.tokens ? `CHAT TOKENS (defined in earlier replies, used by the document - write the token, it is replaced by this text afterwards):\n${a.tokens}` : '',
...
function truncatedObjects(s) {
    ...
    while (i >= 0) {
        ...
        if (!obj) {
            ...
            if (p && pr && pr[1].trim().length >= 20) obj = { p: Number(p[1]), prompt: pr[1].trim(), ...(ar ? { ar: ar[1] } : {}) };
        }
